...
for name, signal in all_signals.items():
    ...
    # Calculate Bandwidth
    power = mag_pos**2
    cum_p = np.cumsum(power)
    total_p = cum_p[-1]
    low = np.searchsorted(cum_p, 0.005 * total_p)
    high = np.searchsorted(cum_p, 0.995 * total_p)
    bw = f_pos[high] - f_pos[low]
    results_bw[name] = bw
    
    # Save Spectrum Plot
    plt.figure(figsize=(10, 6))
    plt.plot(f_pos / 1000, mag_pos, color='red')
    ...
    plt.plot(t_zoom, signal[start_sample:end_sample], color='blue')
...
for name, signal in all_signals.items():
    ...
    # BW (already calculated in the loop above, but we'll re-verify)
    pos_mask = freqs >= 0
    f_p = freqs[pos_mask]
    m_p = mag[pos_mask]
    p_cum = np.cumsum(m_p**2)
    tot_p = p_cum[-1]
    low = np.searchsorted(p_cum, 0.005 * tot_p)
    high = np.searchsorted(p_cum, 0.995 * tot_p)
    bw = f_p[high] - f_p[low]
